...
class AlternatingSolverResult:
    ...
    reference_confusions: Dict[str, np.ndarray]
...
def solve_alternating(
    ...
    *,
    ...
    final_state_primal = np.nan
    final_state_dual = np.nan
    final_state_max_overlap = np.nan
    final_confusion_avg_pg_iters = np.nan
...
def _self_test_fixed_point_identity_case() -> None:
    ...
    for region in cfg.regions:
        name = region.name
        assert np.linalg.norm(result.region_states[name] - sim.region_states[name], ord="fro") <= 1e-7
        assert np.linalg.norm(result.region_confusions[name] - sim.region_confusions[name], ord="fro") <= 1e-7
