...
def _directed_dual_key(
    ...
    return graph.directed_dual_key(source_region, target_region)
...
def partial_trace_adjoint(
    ...
    ----------
    omega :
        Operator on the kept subsystem, with subsystem order matching `keep`.
    dims :
        Full subsystem dimensions.
    keep :
        Indices of the subsystems kept by the forward partial trace.
    ...
    omega = _as_numpy_array(omega, dtype=np.complex128)
    if omega.ndim != 2 or omega.shape[0] != omega.shape[1]:
        raise ValueError(f"omega must be a square matrix, got shape {omega.shape}.")
    ...
    dim_keep = int(np.prod(dims_keep)) if len(dims_keep) > 0 else 1
    dim_trace = int(np.prod(dims_trace)) if len(dims_trace) > 0 else 1

    if omega.shape != (dim_keep, dim_keep):
        raise ValueError(
            f"omega has shape {omega.shape}, expected {(dim_keep, dim_keep)} "
            f"for kept subsystem dimensions {dims_keep}."
        )

    if len(trace_out) == 0:
        return np.array(omega, copy=True)

    perm = tuple(keep) + tuple(trace_out)
    dims_perm = tuple(dims[i] for i in perm)

    ext_perm = np.kron(omega, identity(dim_trace))
...
def initialize_eta_from_region_states(
    ...
    average_pair_reductions :
        If True, initialize eta_{ij} as the average of the two overlap reductions.
        If False, use the reduction from the lower-index region.

    Returns
    -------
    dict
        Mapping canonical overlap key (name_i, name_j) -> eta_{ij}.
    """
    eta: Dict[Tuple[str, str], np.ndarray] = {}
...
def initialize_zero_duals(
    ...
    """
    duals: Dict[Tuple[str, str], np.ndarray] = {}
    ...
    for i, j in graph.overlap_pairs:
        ...
        duals[(name_i, name_j)] = np.zeros((dim_overlap, dim_overlap), dtype=np.complex128)
        duals[(name_j, name_i)] = np.zeros((dim_overlap, dim_overlap), dtype=np.complex128)
...
def validate_eta_collection(
    ...
    for i, j in graph.overlap_pairs:
        ...
        if eta[key].shape != expected_shape:
            raise ValueError(
                f"eta[{key}] has shape {eta[key].shape}, expected {expected_shape}."
            )


def validate_dual_collection(
    graph: RegionGraph,
    duals: Mapping[Tuple[str, str], np.ndarray],
) -> None:
    """
    ...
    for i, j in graph.overlap_pairs:
        ...
        if duals[key_ij].shape != expected_shape:
            raise ValueError(
                f"duals[{key_ij}] has shape {duals[key_ij].shape}, expected {expected_shape}."
            )
        if duals[key_ji].shape != expected_shape:
            raise ValueError(
                f"duals[{key_ji}] has shape {duals[key_ji].shape}, expected {expected_shape}."
            )
...
def region_augmented_state_gradient(
    ...
    grad = np.array(grad_parts["grad_rho"], copy=True)
    grad += gamma_rho * (rho - rho_outer_prev)
    ...
    for neigh_idx in graph.neighbors(region_idx):
        ...
        grad += partial_trace_adjoint(overlap_term, dims=dims, keep=keep_self)
...
def solve_region_state_update_pg(
...
) -> Tuple[np.ndarray, RegionStatePGInfo]:
    """
    ...
    for it in range(1, max_iters + 1):
        ...
        for _ in range(max_backtracking_iters):
            ...
            # Simple projected-Armijo-style acceptance.
        ...
        if not accepted:
            # Fall back to the best projected candidate found if it improves,
            # otherwise stop.
...
def solve_state_subproblem_admm(
    ...
    region_povms: Mapping[str, POVM],
    fixed_confusions: Mapping[str, np.ndarray],
    region_states_outer_prev: Mapping[str, np.ndarray],
    eta_init: Optional[Mapping[Tuple[str, str], np.ndarray]] = None,
    duals_init: Optional[Mapping[Tuple[str, str], np.ndarray]] = None,
    *,
    ...
    region_povms :
        Region-name -> POVM.
    fixed_confusions :
        Region-name -> fixed confusion matrix for the current outer iteration.
    region_states_outer_prev :
        Region-name -> previous outer-iterate states rho^k.
    eta_init :
        Optional initial overlap consensus variables.
    duals_init :
        Optional initial directed dual variables.
    graph :
        Optional prebuilt RegionGraph.
    loss :
    ...
    beta, gamma_rho, inner_max_iters, inner_primal_tol, inner_dual_tol,
    state_step_size, state_gd_max_iters, state_gd_tol, verbose :
        Optional overrides of cfg.admm settings.

    Returns
    -------
    StateADMMResult
        Final inner-ADMM solution and diagnostics.
    """
    graph = RegionGraph(cfg) if graph is None else graph
    ...
    if duals_init is None:
        duals_curr = initialize_zero_duals(graph)
    else:
    ...
    final_primal = np.nan
    final_dual = np.nan
    final_max_overlap = np.nan
    it_used = 0

    if len(graph.overlap_pairs) == 0:
        # No overlaps: solve each region independently once.
        rho_new = {}
        pg_iters = []
        ...
        result = StateADMMResult(
            ...
            history={
                ...
                "average_region_pg_iters": [float(np.mean(pg_iters)) if pg_iters else 0.0],
            },
        )
    ...
    for it in range(1, inner_max_iters + 1):
        ...
        # ----------------------------------------------------
        # 1) Region-state updates (serial execution of a Jacobi-style step)
        # ----------------------------------------------------
        rho_next: Dict[str, np.ndarray] = {}
        ...
        eta_next: Dict[Tuple[str, str], np.ndarray] = {}
        ...
        # 3) Dual updates
        # ----------------------------------------------------
        duals_next: Dict[Tuple[str, str], np.ndarray] = {}
        ...
        history["average_region_pg_iters"].append(
            float(np.mean(region_pg_iters)) if region_pg_iters else 0.0
        )
...
def _self_test_partial_trace_adjoint_identity() -> None:
    ...
    a = rng.normal(size=(dim_full, dim_full)) + 1j * rng.normal(size=(dim_full, dim_full))
    a = hermitian_part(a)

    dim_keep = dims[0] * dims[2]
    omega = rng.normal(size=(dim_keep, dim_keep)) + 1j * rng.normal(size=(dim_keep, dim_keep))
...
def _self_test_initializers() -> None:
    ...
    duals = initialize_zero_duals(graph)
...
def _self_test_state_admm_truth_fixed_point() -> None:
    ...
    dual0 = initialize_zero_duals(graph)
    ...
    # Since we start exactly at a consistent zero-residual fixed point,
    # the solver should stay essentially there.
